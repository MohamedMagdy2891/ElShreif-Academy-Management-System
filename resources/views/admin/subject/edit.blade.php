...
@endphp
<!-- Page Title -->
@section('title', 'تعديل مادة : '.$row->name)

<!-- Header Nav Title -->
@section('nav', 'الرئيسية')

<!-- Header SubNav Title -->
@section('sub_nav', 'تعديل مادة : '.$row->name))
        ...
        <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
            <div class="card">
                <div class="card-header">
                    <div class="card-title">تعديل مادة : {{ $row->name }}</div>
                </div>
                <form class="card-body" action="{{ URL::route('admin.subject.update',$row->id) }}" method="POST">
                    @csrf
                    @method('PATCH')
