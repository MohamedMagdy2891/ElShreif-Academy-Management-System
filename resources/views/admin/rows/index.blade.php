...
@endphp
<!-- Page Title -->
@section('title', 'كل الصفوف')

<!-- Header Nav Title -->
@section('nav', 'الرئيسية')

<!-- Header SubNav Title -->
@section('sub_nav', 'كل الصفوف')
                ...
                <table class="table custom-table m-0">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center">اسم الصف</th>
                        ...
                        @if (count($rows) > 0)
                            ...
                            @foreach ($rows as $row)
                                <tr>
                                    ...
                                    <td class="text-center">{{ count($row->Students) }}</td>
                                    <td class="text-center">{{ $row->User->name }} ( {{ $row->User->role == 1 ? 'مدير النظام' : 'مشرف للنظام' }} )</td>
                                    <td>
                                        <div class="td-actions">
                                            @if (Auth::user()->role == 1)
                                                <a href="{{ URL::route('admin.rows.edit', $row->id) }}"
                                                    class="icon green" data-toggle="tooltip" data-placement="top"
                                                    title="" data-original-title="تعديل الصف">
                                                    <i class="icon-pencil"></i>
                                                </a>
                                                <form action="{{ URL::route('admin.rows.destroy', $row->id) }}"
                                                    method="post">
                                                    @csrf
                                                    @method('delete')
                                                    <button type="submit" class="icon bg-secondary text-light delete"
                                                        data-toggle="tooltip" data-target="tooltip" data-placement="top"
                                                        title="حذف الصف" data-original-title="حذف الف">
